...
export const FeedbackPage = () => {
  ...
  const submitFeedback = async (event: FormEvent) => {
    ...
    if (!isFirebaseConfigured || !currentUid) {
      setError('Feedback submission requires a signed-in Firebase workspace account.');
      return;
    }
...
const getFeedbackError = (error: unknown, fallback: string) => {
  if (typeof error === 'object' && error && 'code' in error) {
    ...
    if (code === 'permission-denied') {
      return 'Feedback could not be saved. Please check workspace permissions or Firestore rules.';
    }
