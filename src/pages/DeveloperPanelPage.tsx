...
export const DeveloperPanelPage = () => {
  ...
  return (
    <main className="min-h-screen max-w-full overflow-x-hidden bg-[#050505] text-[color:var(--color-text-primary)]">
      <div className="pointer-events-none fixed -left-32 -top-32 h-[30rem] w-[30rem] rounded-full bg-[var(--color-accent-14)] blur-[140px]" />
      ...
      <div className="grid min-h-screen max-w-full overflow-x-hidden lg:grid-cols-[minmax(220px,260px)_minmax(0,1fr)]">
        <aside className="relative flex min-w-0 flex-col border-b border-[color:var(--color-border-weak)] bg-[var(--color-overlay-45)] px-3 py-4 backdrop-blur-xl lg:border-b-0 lg:border-r">
          <div className="flex items-center gap-3 px-2 pb-5">
            <MFLogo />
            <div>
              <p className="text-sm font-semibold text-[color:var(--color-text-primary)]">OfficeOS</p>
              <p className="text-xs text-[color:var(--color-text-muted)]">Developer Console</p>
            </div>
          </div>

          <nav className="space-y-1.5 border-t border-[color:var(--color-border-weak)] pt-4">
              ...
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-lg border px-3 py-2.5 text-left text-sm font-medium transition ${
                  activeItem === item.label
                    ? 'border-[color:var(--color-border-weak)] bg-[var(--color-fill-10)] text-[color:var(--color-text-primary)] shadow-[0_0_30px_rgba(226,232,240,0.05)]'
                    : item.enabled
                      ? 'border-transparent text-[color:var(--color-text-secondary)] hover:border-[color:var(--color-border-weak)] hover:bg-[var(--color-fill-045)] hover:text-[color:var(--color-text-primary)]'
                      : 'cursor-not-allowed border-transparent text-[color:var(--color-text-faint)]'
                }`}
              >
                <item.icon size={18} />
          ...
          <button
            type="button"
            onClick={logout}
            className="mt-6 flex w-full items-center gap-3 rounded-lg border border-[color:var(--color-border-weak)] bg-[var(--color-overlay-35)] px-3 py-2.5 text-sm font-medium text-[color:var(--color-text-secondary)] transition hover:border-[color:var(--color-accent-40)] hover:bg-[var(--color-accent-10)] hover:text-[color:var(--color-text-primary)] lg:mt-auto"
          >
            <LogOut size={18} />
            Logout
          </button>
        ...
        <section className="relative min-w-0 overflow-x-hidden px-3 py-4 md:px-5">
          <header className="flex min-w-0 flex-col gap-3 border-b border-[color:var(--color-border-weak)] pb-4 md:flex-row md:items-center md:justify-between">
            <div>
              <h1 className="text-2xl font-semibold tracking-normal text-[color:var(--color-text-primary)]">Developer Panel</h1>
              <p className="mt-1 text-sm text-[color:var(--color-text-muted)]">Feedback & Improvement Center</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center gap-3 rounded-full border border-[color:var(--color-border-weak)] bg-[var(--color-overlay-30)] py-1 pl-1 pr-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-full border border-[color:var(--color-border-strong)] bg-[var(--color-overlay-35)] text-xs font-semibold text-[color:var(--color-text-primary)]">
                  MF
                </div>
                <div>
                  <p className="text-sm font-semibold text-[color:var(--color-text-primary)]">MF Developer</p>
                  <p className="text-xs text-[color:var(--color-text-muted)]">Administrator</p>
                </div>
                <ChevronDown size={15} className="text-[color:var(--color-text-muted)]" />
              </div>
              <button
                type="button"
                onClick={logout}
                className="inline-flex items-center gap-2 rounded-lg border border-[color:var(--color-border-weak)] bg-[var(--color-fill-045)] px-3 py-2 text-xs font-semibold text-[color:var(--color-text-secondary)] transition hover:border-[color:var(--color-accent-40)] hover:bg-[var(--color-accent-10)] hover:text-[color:var(--color-text-primary)]"
              >
                <LogOut size={15} />
                Logout
              </button>
          ...
          <section className="mt-5 max-w-full overflow-hidden rounded-xl border border-[color:var(--color-border-weak)] bg-[var(--color-fill-035)] shadow-[0_24px_70px_rgba(0,0,0,0.38)] backdrop-blur-xl">
            <div className="flex flex-col gap-3 border-b border-[color:var(--color-border-weak)] p-3 xl:flex-row xl:items-center xl:justify-between">
              <div>
                <h2 className="text-base font-semibold text-[color:var(--color-text-primary)]">Recent Feedbacks</h2>
              ...
              <div className="grid min-w-0 gap-2 sm:grid-cols-2 xl:grid-cols-[120px_130px_150px_130px_minmax(170px,1fr)]">
                ...
                <label className="flex min-w-0 items-center gap-2 rounded-lg border border-[color:var(--color-border-weak)] bg-[var(--color-overlay-25)] px-3 py-2 text-sm text-[color:var(--color-text-secondary)]">
                  <Search size={15} className="shrink-0 text-[color:var(--color-text-muted)]" />
                  <input
                    className="min-w-0 w-full bg-transparent outline-none placeholder:text-[color:var(--color-text-faint)]"
            ...
            {error ? (
              <div className="border-b border-[color:var(--color-error-line-20)] bg-[var(--color-error-fill-10)] px-4 py-3">
                <p className="text-sm font-medium text-[color:var(--color-error-text-100)]">{error}</p>
                <p className="mt-1 text-xs leading-5 text-[color:var(--color-error-text-200-75)]">
            ...
            {loading ? (
              ...
              <div className="max-w-full overflow-x-auto">
                <table className="w-full min-w-[920px] text-left">
                  <thead className="border-b border-[color:var(--color-border-weak)] bg-[var(--color-overlay-25)] text-[11px] uppercase tracking-[0.14em] text-[color:var(--color-text-muted)]">
                    <tr>
                    ...
                    {filteredFeedback.map((item) => (
                      <tr key={item.path || item.id} className="transition hover:bg-[var(--color-fill-035)]">
                        <td className="max-w-[280px] px-3 py-3">
                          ...
                          <p className="mt-1 line-clamp-1 text-xs leading-5 text-[color:var(--color-text-muted)]">{item.description}</p>
                        </td>
                        <td className="px-3 py-3">
                          <TypeBadge type={item.type} />
                        </td>
                        <td className="px-3 py-3 text-sm text-[color:var(--color-text-secondary)]">{item.relatedModule}</td>
                        <td className="px-3 py-3">
                          <PriorityBadge priority={item.priority} />
                        </td>
                        <td className="px-3 py-3">
                          <select
                            className="rounded-lg border border-[color:var(--color-border-weak)] bg-[var(--color-overlay-45)] px-2.5 py-1.5 text-xs font-medium text-[color:var(--color-text-soft)] outline-none transition focus:border-[color:var(--color-accent-45)]"
                            value={item.status}
                        ...
                        <td className="max-w-[150px] px-3 py-3 text-sm text-[color:var(--color-text-secondary)]">
                          <p className="truncate">{item.submittedByName}</p>
                          <p className="mt-1 text-xs text-[color:var(--color-text-muted)]">{item.submittedByRole}</p>
                        </td>
                        <td className="px-3 py-3 text-sm text-[color:var(--color-text-secondary)]">{formatDateTime(item.createdAt)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <p className="py-5 text-center text-xs text-[color:var(--color-text-faint)]">OfficeOS Developer Panel by MotoFlexing</p>
        </section>
      </div>
    </main>
  );
...
const DeveloperStat = ({ icon: Icon, title, value }: { icon: typeof MessageSquare; title: string; value: number }) => (
  <article className="rounded-xl border border-[color:var(--color-border-weak)] bg-[var(--color-fill-035)] p-4 shadow-[0_18px_54px_rgba(0,0,0,0.32)] backdrop-blur">
    <div className="flex h-8 w-8 items-center justify-center rounded-lg border border-[color:var(--color-border-weak)] bg-[var(--color-fill-045)] text-[color:var(--color-text-soft)]">
      <Icon size={16} />
    </div>
    <p className="mt-4 text-2xl font-semibold text-[color:var(--color-text-primary)]">{value}</p>
    <p className="mt-1 text-xs text-[color:var(--color-text-secondary)]">{title}</p>
  </article>
);
...
const DeveloperSelect = <Value extends string>({
  ...
  <label className="relative">
    <select
      className="w-full appearance-none rounded-lg border border-[color:var(--color-border-weak)] bg-[var(--color-overlay-25)] px-3 py-2 pr-8 text-xs font-medium text-[color:var(--color-text-soft)] outline-none transition focus:border-[color:var(--color-accent-45)]"
      value={value}
    ...
    <SlidersHorizontal className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[color:var(--color-text-faint)]" size={14} />
  </label>
);
...
const DeveloperEmpty = ({ description, title }: { description: string; title: string }) => (
  <div className="flex min-h-56 items-center justify-center p-6 text-center">
    <div className="max-w-md">
      <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-lg border border-[color:var(--color-border-weak)] bg-[var(--color-fill-045)] text-accent-100">
        <Wrench size={19} />
      </div>
      <h3 className="mt-3 text-base font-semibold text-[color:var(--color-text-primary)]">{title}</h3>
...
const typeClass = (type: FeedbackType) => {
  ...
  if (type === 'Workflow Issue') return 'border-[color:var(--color-warning-line-25)] bg-[var(--color-warning-fill-10)] text-[color:var(--color-warning-text-200)]';
  return 'border-[color:var(--color-neutral-line-25)] bg-[var(--color-neutral-fill-10)] text-[color:var(--color-text-secondary)]';
};

const priorityClass = (priority: FeedbackPriority) => {
  if (priority === 'Critical') return 'border-red-400/30 bg-red-500/12 text-red-200';
  if (priority === 'High') return 'border-accent-400/30 bg-[var(--color-accent-12)] text-accent-100';
  if (priority === 'Medium') return 'border-orange-400/25 bg-orange-500/10 text-orange-200';
  return 'border-[color:var(--color-success-line-25)] bg-[var(--color-success-fill-10)] text-[color:var(--color-success-text-200)]';
};
...
const MFLogo = () => (
  <div className="text-4xl font-black italic tracking-[-0.08em] drop-shadow-[var(--shadow-glow-22-25)]">
