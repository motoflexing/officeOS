...
import { firestoreService, type DashboardPendingActions } from '../services/firestoreService';
...
export const DashboardPage = () => {
  ...
  const [pendingActions, setPendingActions] = useState<DashboardPendingActions | null>(null);
  const [attendanceError, setAttendanceError] = useState('');

  const canReviewActions = currentRole === 'Admin' || currentRole === 'HR';
  ...
  useEffect(() => {
    ...
    if (!isFirebaseConfigured) {
      setPendingActions(storage.getDashboardPendingActions());
      return;
    }
        ...
        <section className="surface border-accent-500/25 p-5 shadow-[0_0_34px_rgba(239,35,43,0.12)]">
          ...
          <div className="mt-4 divide-y divide-white/10 overflow-hidden rounded-lg border border-white/10">
            <PendingActionRow
              ...
              count={pendingActions.pendingLeave}
            />
            <PendingActionRow
              to="/reports"
              icon={FileText}
              label="Unreviewed daily reports today"
              count={pendingActions.unreviewedReports}
            />
            <PendingActionRow
              to="/hr"
              icon={CalendarClock}
              label="Interviews scheduled today"
              count={pendingActions.interviewsToday}
            />
          </div>
        </section>
      ) : null}
